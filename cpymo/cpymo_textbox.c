...
void cpymo_textbox_show_next_char(cpymo_textbox *tb)
{
    ...
    cpymo_str span = tb->text_curline_and_remaining;
    ...
    if (cpymo_str_equals_str(next_char, "\\")) {
        next_char = cpymo_str_utf8_try_head(&span);
        if (cpymo_str_equals_str(next_char, "n")
            || cpymo_str_equals_str(next_char, "r")) {
            tb->text_curline_size += next_char.len;
            cpymo_textbox_show_next_line(tb);
        }
    }
    else if (cpymo_str_equals_str(next_char, "\n") || cpymo_textbox_line_full(tb)) {
        cpymo_textbox_show_next_line(tb);
    }
}
