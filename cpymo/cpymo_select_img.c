...
error_t cpymo_select_img_configuare_begin(
	...
	cpymo_str image_name_or_empty_when_select_imgs,
...
error_t cpymo_select_img_configuare_select_imgs_selection(cpymo_engine *e, cpymo_str image_name, float x, float y, bool enabled, uint64_t hash)
{
	assert(e->select_img.selections);
	assert(e->select_img.all_selections);
...
static bool cpymo_select_img_wait(struct cpymo_engine *e, float dt)
{
	...
	if (e->select_img.save_enabled) {
		...
		else if (CPYMO_INPUT_JUST_RELEASED(e, cancel) || mouse_button_state == cpymo_key_hold_result_just_hold)
			cpymo_rmenu_enter(e);
	}
...
error_t cpymo_select_img_configuare_select_text(
	...
	cpymo_str text, bool enabled, enum cpymo_select_img_selection_hint_state hint_mode, 
...
void cpymo_select_img_configuare_select_text_hint_pic(cpymo_engine * engine, cpymo_str hint)
{
	...
	for (size_t i = 0; i < 4; ++i) {
		cpymo_str_copy(hint_pic_name, hint.len + 2, hint);
